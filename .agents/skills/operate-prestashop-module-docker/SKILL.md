...
Expect first runs to create ignored dependency directories such as `tooling/vendor/` and `phpunit/vendor/`. Dependencies are installed automatically when needed, but existing installations are not synchronized after changing a Composer manifest, lock file, or container PHP version. Reinstall the affected dependencies from the current lock file before validating the change. If dependency resolution fails, derive PHP extension and version requirements from the current Composer manifests and lock files, then adjust the relevant image or dependency set rather than applying historical assumptions.
